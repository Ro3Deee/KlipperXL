...
# All rights reserved.
#
# This software was written by Richard Crook for the XlKlipper project.
# Unauthorized copying or claiming of authorship is prohibited.
# License: GPLv3
...
class PuppyBootloader:
    ...
    def _check_side_autoload(self, tool, old_state, new_state):
        ...
        # Detect no_filament -> has_filament transition (debounce with 2 readings)
        if new_state == "has_filament" and old_state in ("no_filament", "not_calibrated"):
            count = self._autoload_insert_count.get(tool, 0) + 1
            self._autoload_insert_count[tool] = count
            if count >= 2:
                self._autoload_insert_count[tool] = 0
                logging.info(f"PuppyBootloader: Side sensor T{tool} insertion "
                           f"detected - starting autoload")
                self.gcode.respond_info(
    ...
    def _do_autoload(self, tool):
        ...
        Sequence matching Prusa stock firmware:
        1. Pick tool if not active
        2. Slow load 30mm @ 5.4mm/s (cold OK) - pull to gears
        3. Heat to 170C minimum (or current target)
        4. Fast load 50mm @ 18mm/s - fill hotend
        5. Purge 27mm @ 2.7mm/s - prime nozzle
        6. Retract 4mm @ 35mm/s - prevent ooze
        """
        dwarf = tool + 1
        if dwarf not in self.booted_dwarfs:
            self.gcode.respond_info(f"T{tool}/Dwarf {dwarf} not booted - cannot autoload")
            return
        ...
        # Slow load to gears: 5.4mm/s = 324mm/min, 30mm (cold extrusion OK)
        self.gcode.respond_info(f"T{tool}: Slow loading to gears...")
        ...
        self.gcode.run_script_from_command("M302 S170")      # Restore min temp

        # Check tool sensor - did filament reach the gears?
        data = self.dwarf_data.get(dwarf, {})
        raw = data.get('filament_sensor', 0)
        state = self._evaluate_filament_state(raw, tool)
        if state != "has_filament" and state != "not_calibrated":
            self.gcode.respond_info(
                f"T{tool}: Filament not detected at tool sensor after slow load. "
                f"Push filament further and try LOAD_FILAMENT TOOL={tool}")
            return

        # Determine temperature - minimum 170C for extrusion
        temp = self.target_temps.get(dwarf, 0)
        if temp < 170:
            temp = 200  # Default load temp
        self.gcode.respond_info(f"T{tool}: Heating to {temp}C...")
        self.gcode.run_script_from_command(f"M109 T{tool} S{temp}")

        # Fast load: 18mm/s = 1080mm/min, 50mm (fill hotend)
        self.gcode.respond_info(f"T{tool}: Fast loading...")
        self.gcode.run_script_from_command("G92 E0")
        self.gcode.run_script_from_command("G1 E50 F1080")
        ...
        # Turn off heater (idle autoload, no print pending)
        self.gcode.run_script_from_command(f"M104 T{tool} S0")
